...
// findInPath searches for an executable in the given PATH string.
// On Windows, also checks common executable extensions (.exe, .cmd, .bat).
func findInPath(pathEnv, name string) (string, error) {
	...
	for _, dir := range strings.Split(pathEnv, string(os.PathListSeparator)) {
		...
		// Try bare name and common Windows extensions
...
// exitStatus extracts the integer exit code from an exec error.
// Uses ExitCode() which is cross-platform (Go 1.12+).
