...
func buildEntry(draft *tui.Draft, existing *scheduler.ScheduleEntry) (scheduler.ScheduleEntry, error) {
	...
	usr, _ := user.Current()
	// On Windows, USERNAME is the standard env var (USER is Unix-only).
	username := os.Getenv("USERNAME")
	if username == "" {
		username = os.Getenv("USER")
	}
	if usr != nil && usr.Username != "" {
		// Strip domain prefix (e.g. "MACHINE\User" → "User")
		u := usr.Username
	...
	// On Windows, os.Getuid/Getgid return -1; UID/GID fields are kept for
	// JSON schema compatibility but are not used for privilege operations.
	uid := os.Getuid()
	gid := os.Getgid()
	...
	if pathEnv == "" {
		// Build a sensible Windows fallback PATH covering Node.js (Claude CLI) locations.
		...
		parts := []string{}
		if localAppData != "" {
			parts = append(parts, filepath.Join(localAppData, "Programs", "nodejs"))
		}
		if appData != "" {
			parts = append(parts, filepath.Join(appData, "npm"))
		}
		parts = append(parts,
